...
use echo_shared::{
    ...
    WebSocketMessage, AudioFormat, now_utc
};
...
impl EchoKitClient {
    ...
    pub async fn send_message(&self, message: EchoKitClientMessage) -> Result<()> {
        ...
        if let Some(ws_stream) = self.ws_stream.read().await.as_ref() {
            let mut stream_clone = ws_stream.clone();
            tokio::spawn(async move {
                if let Err(e) = stream_clone.send(Message::Text(json_message)).await {
            ...
            loop {
                tokio::select! {
                    ...
                    Some(message_result) = async {
                        if let Some(stream) = ws_stream.read().await.as_ref() {
                            stream.next().await
                        } else {
                            None
                        }
                        ...
                        match message_result {
                            ...
                            Ok(Message::Close(_)) => {
                                warn!("EchoKit Server closed connection");
                    ...
                    Some(message) = receiver.recv() => {
                        if let Some(stream) = ws_stream.read().await.as_ref() {
                            ...
                            if let Err(e) = stream.clone().send(Message::Text(json_message)).await {
                                error!("Failed to send message to EchoKit Server: {}", e);
                                *is_connected.write().await = false;
                                break;
                            }
                    ...
                    _ = tokio::time::sleep(tokio::time::Duration::from_secs(30)) => {
                        if let Some(stream) = ws_stream.read().await.as_ref() {
                            if let Err(e) = stream.clone().send(Message::Text(r#"{"type":"Ping"}"#.to_string())).await {
